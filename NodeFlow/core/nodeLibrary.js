...
const normalizeConstants = (constants) => {
  ...
  return constants
    .map((constant) => {
      ...
      seen.add(key);
      const labelSource =
        typeof constant.label === 'string' && constant.label.trim()
          ? constant.label.trim()
          : rawKey || key;
...
export const SAMPLE_NODE_TEMPLATES = [
  {
    ...
    constants: [
      { key: 'message', label: 'Log message', default: '"Hello from custom node"' },
    ],
    script: [
  ...
  },
  {
    ...
    constants: [
      {
        key: 'castAsInt',
        label: 'Cast inputs to [int] before adding?',
...
export const createEmptySpec = () => ({
  ...
  outputs: [],
  constants: [
    { key: 'note', label: 'Notes', default: '# TODO: describe behavior' },
  ],
  script: [
